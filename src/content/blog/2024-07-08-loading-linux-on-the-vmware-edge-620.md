...
After this you will find yourself in a very normal looking BIOS setup menu. The first time I experienced this, it was a bit surprising to me, being that we are connecting over a serial connection. On my Mac the selected text does not show up so this makes it a little tricky to navigate, but still doable.

Once you enter the BIOS setup utility, use right arrow to move over to the "Advanced" tab, use the down arrow to select CSM Configuration and hit enter. Hit enter again and choose Disabled, hit the escape key, them move over to the "Save & Exit" tab and choose Save Changes and Exit. After you end up back at the option, press delete to enter the BIOS again and arrow over to the "Save & Exit" tab, find and choose the UEFI version of USB drive under the "Boot Override" section before the hardware watchdog reboots the box!

Disabling the CSM will prevent some issues that can happen when we try and install Dell DiagOS on the box.
